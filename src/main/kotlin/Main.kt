...
fun main() {
    println("Hola, soy tu bola 8 magica, Cual de estas opciones deseas realizar")
...
fun mostrarError() {
    println("ELEGISTE UNA RESPUESTA NO VALIDA, HASTA PRONTO")
}
...
fun mostrarRespuestas() {
    ...
    println("2. Revisar solo las afirmativas")
    println("3. Revisar solo las negativas")
    println("4. Revisar solo las dudosas")
    val opcionIngresada = readLine()
    when(opcionIngresada){
        "1" -> mostrarRespuestasTipo()
        "2" -> mostrarRespuestasTipo(tipoRespuesta = RESPUESTA_AFIRMATIVA)
        "3" -> mostrarRespuestasTipo(tipoRespuesta = RESPUESTA_NEGATIVA)
        "4" -> mostrarRespuestasTipo(tipoRespuesta = RESPUESTA_DUDOSA)
        else -> println("Respuesta no valida")
    }
}

fun mostrarRespuestasTipo(tipoRespuesta : String = "TODOS"){
    when(tipoRespuesta){
...
fun realizarPregunta(){
    ...
    readLine()
    println("Asi que esa era tu pregunta... la respusta a eso es -> ")
